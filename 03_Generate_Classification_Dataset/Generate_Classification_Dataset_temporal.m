...
for session = 1 : numel(sessionPaths)
    ...
    for idx_range = 1 : size(ranges, 1)
        ...
        for i = 1 : size(eventDataRaw,1)
            if ... % Robot session + P 
                ...
                if eventDataRaw.Time_ms(i) - eventDataRaw.Time_ms(i-1) > 5000
                    if i < size(eventDataRaw,1)
                        if eventDataRaw.Time_ms(i+1) - eventDataRaw.Time_ms(i) > 5000
        ...
        save(fullfile(tankPath,"RobotNP_RobotP_ITI_ITI_" + num2str(ranges(idx_range, 1)/1000) + "_" + num2str(ranges(idx_range, 2)/1000) + ".mat"), "X", "y", "region");
    end
