...
################################
single_unit_datapoint = 20  # 4 sec * 100ms bins = 40 features per single unit
...
if __name__ == "__main__":
    ...
    def write_region_csv(out_path, region_idx_acc, region_idx_null):
        # Layout: Session, T1_Random, T2_Random, ..., T10_Random, T1_Real, T2_Real, ..., T10_Real
        header = ['Session']
        ...
        for session_name in sorted(results.keys()):
            ...
            row = [session_name]
            # First pass: all Random values
            for w_idx in range(len(TIME_WINDOWS)):
                acc_b, null_b, acc_p, null_p = session_data[w_idx]
                null_val = (acc_b, null_b, acc_p, null_p)[region_idx_null]
                row.append(f"{null_val:.4f}")
            # Second pass: all Real values
            for w_idx in range(len(TIME_WINDOWS)):
                acc_b, null_b, acc_p, null_p = session_data[w_idx]
                acc_val = (acc_b, null_b, acc_p, null_p)[region_idx_acc]
